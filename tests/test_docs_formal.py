...
What a reader must not break:
  ...
  - `SCANNED_EXCLUSIONS` is the corpus boundary and every member is excluded for a stated reason
    (`the theory chapters`, *The registry*). Widening the corpus is a one-line change; narrowing it
...
EVIDENCE = DOCS_DIR / "theory" / "08-evidence.md"
...
#: `the theory chapters` states; none is excluded because keying it was inconvenient.
SCANNED_EXCLUSIONS = (
...
def _theory_documents() -> list[Path]:
    return sorted(THEORY.glob("*.md"))


def _document() -> str:
    assert EVIDENCE.is_file(), f"{EVIDENCE} does not exist"
...
def _section(title: str, path: Path = EVIDENCE) -> str:
...
def test_theory_index_links_the_evidence_chapter():
    """The mathematical chapters are discoverable from the documentation index."""
...
def test_every_test_named_in_the_formal_doc_exists():
    ...
    assert not missing, (
        "the theory chapters names test(s) that do not exist in tests/: "
        + ", ".join(missing)
...
def test_the_formal_doc_states_the_chain_the_code_defines():
    ...
    assert chain in _document(), (
        "the theory chapters do not state the `Strength` chain; it must read: " + chain
    )
...
def test_the_formal_doc_states_the_basis_rungs_the_code_defines():
    ...
    for basis, rungs in BASIS_RUNGS.items():
        ...
        assert row in document, (
            f"the theory chapters §4.3 does not carry the row for {basis.value}: {row}"
        )
...
def test_the_formal_doc_names_the_algebras_the_code_defines():
    ...
    assert set(rows) == set(ALGEBRAS), (
        "the theory chapters §5.2 does not name the algebras the code defines: "
        f"document {sorted(rows)} vs code {sorted(ALGEBRAS)}"
    )


def test_the_formal_doc_states_one_soundness_paragraph_per_engine():
    """The procedures chapter names every engine under `engines/`, so a new one
    cannot ship without a statement."""
    engines = sorted(
...
def test_the_bibliography_has_entries_and_they_are_unique():
    ...
    assert keys, "theory/bibliography.md defines no bibliography entry"
...
def test_every_citation_key_resolves_to_a_bibliography_entry():
    ...
    assert not unresolved, (
        "citation key(s) with no bibliography entry in theory/bibliography.md: "
...
def test_a_source_named_without_a_key_is_refused():
    ...
    assert not offenders, (
        ...
        + ". Add a `[key]` beside it and a bibliography entry in theory/bibliography.md."
    )
