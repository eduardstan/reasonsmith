...
- **Breaking:** **`claimed_semantics` is a closed vocabulary, and a declaration outside it is refused where it is written.**
  `spec.CLAIMED_SEMANTICS` has three members — `distribution semantics`, `weighted sum`, `free-text rationale` — and `spec.normalize_claimed_semantics` raises for anything else (`ValueError` outside the vocabulary, `TypeError` for a non-string). Claimed semantics are canonicalized at the shipped artefact boundaries (`artifacts/ground_program.py`, `artifacts/reason_trace.py`), reference semantics at `artifacts/__init__.py`, and both fields again on a public `Certificate`; `semantic_laws` likewise consumes and records the canonical member rather than a raw accepted spelling. It is breaking for an out-of-tree artefact family that declared its own prose label: that family now stops constructing and has to move to a member of the vocabulary, or carry its label somewhere other than this field. Nothing shipped here declared one, so no shipped verdict moved. The reason to close it is that a reader comparing two declarations should not have to read an author's prose as a new, interpretable semantics — and the closure is what makes the refusal in the `gdpr_recital71_error_risk_minimised` entry below *not evaluated* rather than an accusation: the vocabulary deliberately has no member for a documented approximation *of* a member, so a system that truncates honestly cannot claim the one semantics this build computes a reference for, and the duty leaves it unjudged. [`docs/semantics.md`](docs/semantics.md) states the vocabulary and `tests/test_claimed_semantics.py` holds it, including that each of the two artefact families shipped here exposes only a canonical member and refuses anything else — a per-family check, not an inventory of the tree. ([#156](https://github.com/eduardstan/reasonsmith/pull/156))
