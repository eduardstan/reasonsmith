...
- **The README is reorganised around the five audience reading paths, and what it carried moved rather than went.**
  The front page was 382 lines longer (577 down to 195) and ordered by the project's own history; it is now the organising question, one measured run with its transcript, install, the limits, and a reading path per stakeholder — `developer`, `deployer`, `auditor`, `regulator`, `affected-individual` — each a route through the documents that already existed. Nothing was deleted without a destination: [`docs/README.md`](docs/README.md) is rewritten as the one-line-per-document index the routes point into, and the theory, the state-of-the-art comparison and the roadmap summary are read from [`docs/semantics.md`](docs/semantics.md), [`docs/refinement.md`](docs/refinement.md), [`docs/sufficient-reasons.md`](docs/sufficient-reasons.md) and [`ROADMAP.md`](ROADMAP.md), which gained the paragraphs that had only existed on the front page. Two pointers this reorganisation stranded are repaired here: the committed agent memory (`AGENTS.md`, with `CLAUDE.md` a symlink to it) named README sections that no longer exist, and now names **Install and run** for user-facing CLI usage and [`RESULTS.md`](RESULTS.md) for the `torch`-is-not-a-declared-dependency caveat, which is where that caveat's measurements have always lived. The CLI transcripts on the page are still generated by `python docs/build_readme_transcripts.py` and still held to it byte-for-byte. ([#160](https://github.com/eduardstan/reasonsmith/pull/160))
