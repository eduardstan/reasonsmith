...
- The organising diagram now keeps its decision nodes and edge destinations legible in GitHub dark mode.
...
- **Breaking:** **`claimed_semantics` is a closed vocabulary, and a declaration outside it is refused — at a boundary that differs by family, so an adopter should know which one they will meet.**
  `spec.CLAIMED_SEMANTICS` has three members — `distribution semantics`, `weighted sum`, `free-text rationale` — and `spec.normalize_claimed_semantics` raises for anything else (`ValueError` outside the vocabulary, `TypeError` for a non-string). Where that fires is not uniform, and the difference is the whole migration story. `artifacts/reason_trace.py` normalizes in `__init__`, so a `ReasonTraceArtifact` with a prose label **stops constructing** — the adopter learns at the call that builds it. `artifacts/ground_program.py` normalizes in the `claimed_semantics` **property**, so an adapter carrying a prose label and the artefact wrapping it both construct, and the refusal fires when the value is *read* — which, in an audit, is inside `certificate.certify_artifact`. A family implementing the `artifacts.InferenceArtifact` protocol directly is normalized at neither: it constructs, and is refused at `Certificate.__post_init__`, i.e. partway through the audit. In every one of those in-audit cases `engines/certificate.py` reports that decision *not evaluated*, naming the exception — the run is not aborted and no verdict is invented — so an out-of-tree family that declared its own prose label learns at audit time rather than at import time unless it uses the reason-trace family. `semantic_laws` consumes and records the canonical member rather than a raw accepted spelling, and an artefact exposing no declaration at all is still told why the laws do not apply rather than raised at. The *reference* side is deliberately not symmetric: `artifacts.reference_semantics` canonicalizes an accepted spelling and **carries** an unrecognised one, because that name is not the audited system's claim but a name for what an artefact family's own `exact_value()` computes, and the outcome owed for one this build cannot compare is the mismatch refusal below — *not evaluated*, naming both names. Nothing shipped here declared a label outside the vocabulary, so no shipped verdict moved. The reason to close it is that a reader comparing two declarations should not have to read an author's prose as a new, interpretable semantics — and the closure is what makes the refusal in the `gdpr_recital71_error_risk_minimised` entry below *not evaluated* rather than an accusation: the vocabulary deliberately has no member for a documented approximation *of* a member, so a system that truncates honestly cannot claim the one semantics this build computes a reference for, and the duty leaves it unjudged. [`docs/semantics.md`](docs/semantics.md) states the vocabulary and `tests/test_claimed_semantics.py` holds it, including that each of the two artefact families shipped here exposes only a canonical member and refuses anything else — a per-family check, not an inventory of the tree. ([#156](https://github.com/eduardstan/reasonsmith/pull/156))

- **The margin question the trace rung left open is settled, and the refusal is pinned to carry no margin.**
  The refusal itself is 0.8.0's — a duty whose formula rtamt is known to misread is *not evaluated* naming the construct, rather than answered off a misread formula — and this release does not change that behaviour. The rendering boundary is recorded in [`docs/theory/03-semantics.md`](docs/theory/03-semantics.md), Remark 3.3; since the verdict is this package's own and only the margin is the monitor's, such a duty *could* instead be answered from `eval_temporal_trace` with `evaluation_scores` suppressed — a verdict without a margin rather than no verdict. That is now settled against, with the reasoning written down: a result carrying no margin still claims the observed rung evaluated the duty while the only monitor margin available was computed for another formula, so the refusal keeps the verdict and the misleading margin out of the record together. `test_a_duty_using_a_misread_shape_is_not_evaluated_and_names_the_construct` now also asserts that no `evaluation_scores` accompanies a refused shape, so the settled answer is pinned rather than only stated. **No engine, verdict, rung, pack or shipped document output moved.** ([#161](https://github.com/eduardstan/reasonsmith/pull/161))

- **The README is reorganised around the five audience reading paths, and what it carried moved rather than went.**
  The reorganisation itself took the front page from 578 lines to 195, a net 383 removed — later work in this release adds a couple of lines back, so read those figures as [#160](https://github.com/eduardstan/reasonsmith/pull/160)'s own diff rather than as a measurement of the released tree. The page was ordered by the project's own history; it is now the organising question, one measured run with its transcript, install, the limits, and a reading path per stakeholder — `developer`, `deployer`, `auditor`, `regulator`, `affected-individual` — each a route through the documents that already existed. Nothing was deleted without a destination: [`docs/README.md`](docs/README.md) is rewritten as the one-line-per-document index the routes point into, and the theory, the state-of-the-art comparison and the roadmap summary are read from [`docs/semantics.md`](docs/semantics.md), [`docs/refinement.md`](docs/refinement.md), [`docs/theory/07-explanation.md`](docs/theory/07-explanation.md) and [`ROADMAP.md`](ROADMAP.md), which gained the paragraphs that had only existed on the front page. Two pointers this reorganisation stranded are repaired here: the committed agent memory (`AGENTS.md`, with `CLAUDE.md` a symlink to it) named README sections that no longer exist, and now names **Install and run** for the install and the one worked `check` invocation, [`docs/adopting.md`](docs/adopting.md) and [`docs/authoring-packs.md`](docs/authoring-packs.md) for the rest of the user-facing usage, and [`RESULTS.md`](RESULTS.md) for the `torch`-is-not-a-declared-dependency caveat, which is where that caveat's measurements have always lived. `--system-module` **imports and executes** the module it names, and that sentence is now on the front page beside the command rather than only in `--help` and [`docs/three-systems.md`](docs/three-systems.md). The CLI transcripts on the page are still generated by `python docs/build_readme_transcripts.py` and still held to it byte-for-byte. ([#160](https://github.com/eduardstan/reasonsmith/pull/160))

- **`gdpr_recital71_error_risk_minimised` now reads an approximation error reasonsmith measures, not one the system declares.**
  The duty compared `scope_statements_declared_deviation` — a number a system wrote about its own inference into its own record — against that decision's own margin, and [`docs/findings-nesyarena.md`](docs/findings-nesyarena.md) finding 1 said what that was worth: *it rewards the measurement, not the accuracy*. Its left-hand side is now `engines.certificate.SEMANTICS_VALUE_GAP`, the absolute distance between the system's own engine's answer and exact inference's answer to the same query on the same interpretation, both computed by reasonsmith from the inference artefact `artifact()` returns. This is the repair of an existing clause and not a new one: same Recital, same quotation, same property shape, with the self-declaration overwritten by a measurement — literally the move `engines/certificate._env` already made for `artifact_logs_deleted_reason_count`. `report._engine_ladder` and `report.evidence_basis` now key on `MEASURED_SIGNALS` rather than on one signal name, so the duty has the `artifact` basis and a ladder of one rung and can never be answered off a log; `engines/certificate.py` reduces a top-level `always(f)` through `engines.temporal.state_property_under_always`, so one spelling of that operator exists. This closes objective 5 of [`ROADMAP.md`](ROADMAP.md) for one artefact family: `test_two_systems_differing_only_in_their_inference_get_different_verdicts` drives two systems identical in every respect a report can see and differing only in whether the engine behind `artifact()` implements what it declares, and they now come back `satisfied` at `probed` against `violated` at `probed`.
  **A2 also measures the bound when the artefact exposes it.** An optional finite `decision_threshold` makes the engine compare the measured value gap against `abs(engine_value - decision_threshold)`, ignoring the record's declared margin for that decision; absent thresholds retain the old record-margin behavior, while malformed thresholds, non-finite answers, and non-finite derived margins are not evaluated. `test_an_exposed_threshold_replaces_a_generous_declared_margin` is the falsifying check: exact and miscalibrated systems both pass with a flattering declared margin, and only the measured smaller margin makes the latter violated. This is a threshold-derived margin bound, not a separate sign-flip verdict; the existing `artifact_logs_decision_margin` capability remains the reach gate.

  **What refuses, and why each refusal is the outcome it is.** `artifacts.reference_semantics` names which member of `spec.CLAIMED_SEMANTICS` a family's own `exact_value()` computes — `distribution semantics` for the ground program, `None` for a recounted reason trace, whose exact side is the weight the system itself supplied and whose gap therefore measures a rationale's faithfulness, a different question that looks identical as a number. `artifacts.semantics_reference_refusal` words both refusals once and they land on opposite outcomes: a family computing no reference at all is **unattainable** (the gap is in the system — expose a model encoding), and an artefact claiming a semantics the reference does not match is **not evaluated** naming the claim (the gap is in this tool), per [`docs/semantics.md`](docs/semantics.md) §4. That second refusal is why the closed vocabulary of [#156](https://github.com/eduardstan/reasonsmith/pull/156) had to land first: it has no member for a documented approximation *of* a member, so a system that honestly documents its own truncation cannot declare the one semantics this build computes a reference for, and the duty leaves it unjudged rather than comparing it against a semantics it never claimed. `test_an_honestly_declared_approximation_is_not_accused` runs the *same* deviating engine as the violation test with one attribute changed and holds it to `inconclusive`, `strength=None`. The monotonicity refusals of `artifacts.deletion_semantics_refusal` are now asked only of the deleted-count duty, because that declaration is the premise `deleted` rests on and the value gap is read at the unperturbed interpretation, where nothing has been switched off.
  **The reach is one artefact family and every verdict that moved is named.** A log-only system, the language-model adapter, a recounted reason trace and `reasonsmith.examples.symbolic_rules` are all `unattainable` on this duty — the last of those lost a `proved` verdict, which objective 1 of the roadmap records. In [`docs/nesyarena-conformance-report.md`](docs/nesyarena-conformance-report.md), regenerated here, all five provenances move to `unattainable`: `exact-wmc` and `top-3-proofs` from `satisfied`, `add-mult(clamped)` from `satisfied`, and **`top-1-proofs` and `min-max-prob` from `violated`**. That last pair is the cost, stated rather than buried: their violations rested on a deviation the harness computes with an exact oracle and writes into its own log, which finding 5 of the same document says a deployed system does not have. They are no longer falsely cleared on this duty and they are not yet caught by it, and the roadmap says which change would catch them. The gate's reach is stated in the pack description, in [`docs/semantics.md`](docs/semantics.md) §3 and in column four of [`docs/refinement.md`](docs/refinement.md), because a duty that silently answered only where it could measure while looking like it answered everywhere would be worse than one that refuses out loud. `Certificate` gains `exact_semantics`, carried into `to_dict()` and the per-decision certificate record so a reader of a value gap can see what the engine was compared against; `JSON_SCHEMA_VERSION` is unmoved, both keys being additive. `tests/test_deviation_duty.py` is rewritten around the measurement and holds all of it. ([#162](https://github.com/eduardstan/reasonsmith/pull/162))

- **Paired replay no longer clears a counterfactual duty from partial evidence.**
  A `counterfactually_invariant(o, p)` search used to return `satisfied` at `probed` when at least one planned protected twin completed without changing `o`, even if another planned pair raised; the result counted the errors in its budget while still clearing the duty. It now returns *not evaluated* (`inconclusive`, `strength=None`, reason `some_replays_failed`) whenever any planned pair raises on the satisfaction path. A reproducible pair whose outcomes differ still establishes a violation, because that existential witness does not require every other pair to answer; it is only the universal bounded clearance that needs complete evidence. The change affects systems whose `decide()` accepts some protected twins and fails on others: they are no longer cleared over the subset that answered, and a proof-rung verdict beside that refusal receives no cross-rung cause inferred from a partial `probed` satisfaction. This is the same completeness rule the ordinary replay search already applies. [`docs/semantics.md`](docs/semantics.md) states it for paired replay and `test_partial_replay_evidence_never_becomes_satisfaction_or_a_cross_rung_cause` holds both the verdict and the absent diagnostic. The count-and-pass behaviour it replaces is [#102](https://github.com/eduardstan/reasonsmith/pull/102)'s and predates 0.8.0; this change was made in the v0.9.0 release sweep rather than in a feature pull request, and is recorded without a link for that reason.

### Added

- **A terminal interface and recording harness join the release.** `packages/tui/` is an OpenTUI renderer over one `python -m reasonsmith.cli check ... --json` subprocess result; it carries no second pack, engine or verdict implementation. `packages/terminal-control/` records sessions to `.termctrl` and renders stills, GIF, video and `screen.json` with a SHA-256 manifest, while `packages/cli/` is a thin launcher. These packages are outside the Python distribution built from `src/reasonsmith/`; they require Bun `1.3.14` (checked by the TypeScript workflow). Contributed by `nikomatt69` ([#129](https://github.com/eduardstan/reasonsmith/pull/129)).

- **A model-free autoformalisation harness, a gold corpus covering every shipped requirement, and an optional model boundary in front of both.**
  Writing a clause of law as a `rulelang` formula was a judgement recorded in [`docs/refinement.md`](docs/refinement.md) and checked by nobody but its author. `src/reasonsmith/autoformalize.py` checks a *candidate* formula against the shipped one by two independent gates and constructs no verdict of any kind: `round_trip_check` compares the two denotations and returns `equivalent`, `stronger`, `weaker`, `incomparable` or `refused`; record and logical comparisons use the same Z3 encoding `analysis.py` already uses and carry a solver witness for every non-equivalent answer, temporal comparisons return the entailment relation without a witness, and differing counterfactual atoms return an incomparable finding naming the outcome/protected-signal mismatch. `check_challenges` evaluates the candidate on that requirement's gold cases. `signoff` reports the human sign-off row in the refinement record and never reads a pending sign-off as approval; nothing in the module calls an engine, builds a `RequirementResult` or touches a conformance report. The corpus is `src/reasonsmith/challenges/manifest.toml` at schema version 2, and it covers **every** shipped requirement: record and logical cases carry `signals`, temporal cases carry finite `trace` records and the counterfactual case carries `pairs` of left/right executions, so the two fragments whose objects are not one decision record are checked as themselves rather than skipped. `src/reasonsmith/proposer.py` is the optional model-facing side — the `proposer` extra declares no Python dependency, exactly as `ltlf` declares none, because the provider is installed separately — and it is deliberately thin: it asks for one formula, hands the text to the gates, and retries a failed candidate within a fixed attempt budget, handing back only the round-trip finding, with its witness where available, and the failing gold cases. It never rewrites a formula, and its four outcomes are kept apart on purpose — `unavailable` (no provider reachable), `refused` (a response the repository parser does not accept as one complete formula), `wrong` (parseable but failing a gate) and agreement — so a missing provider can never be counted as a disagreement. Ollama is reachable without a Python client, the installed Claude Code CLI through `ClaudeModel` (`python -m reasonsmith.proposer --claude`), and any other provider by implementing the one-call `Model` protocol or by `--command`. [`docs/autoformalization.md`](docs/autoformalization.md) is the contract and [`RESULTS.md`](RESULTS.md) carries the measurements, with the tiny-model smoke row (0/24, one attempt) kept separate from the first capability measurement (**27/29 machine-cleared, 2 wrong, 0 refused, 0 unavailable**, budget fixed at two attempts before the run). A machine-cleared candidate is a *proposal for a human*, never an accepted formula: no shipped duty changed, and no verdict, rung or engine moved. ([#148](https://github.com/eduardstan/reasonsmith/pull/148), [#150](https://github.com/eduardstan/reasonsmith/pull/150), [#151](https://github.com/eduardstan/reasonsmith/pull/151), [#152](https://github.com/eduardstan/reasonsmith/pull/152), [#154](https://github.com/eduardstan/reasonsmith/pull/154))

- **`reasonsmith published-counts` emits the tree's own counts as JSON, with provenance for every date.**
  The numbers a reader meets first — how many packs, how many requirements, how many verbatim statutory quotes, how many rungs — were prose, retyped wherever they were shown. The subcommand computes them from the shipped packs and the strength enum (`--output` writes to a file, default stdout) at `schema_version` 1, and it carries its own provenance rather than a claim: `generated_at` is when the artefact was made, `source` names what it was derived from, and the quote-verification fields are grounded in an actual run — `python -m reasonsmith.drift --verification-manifest PATH` writes `docs/legal-verification.json` only on a run with no drift. That manifest is schema version 2 and binds the pack id, requirement id and exact UTF-8 quote text with a deterministic SHA-256 digest, so `published_counts()` **refuses** to emit rather than report a stale clearance when the corpus changed without changing its count. With no manifest on disk the fields read `not_run` and `null`, never "verified". `docs/published-counts.json` is the committed artefact and `tests/test_published_counts.py` holds it to the tree, comparing every key but the generation timestamp. It reads no system, runs no engine and changes no verdict. ([#149](https://github.com/eduardstan/reasonsmith/pull/149))

- **A failed certificate measurement is surfaced beside the duty it was measured for.**
  A per-decision certificate could come back `FAIL` — a reason the system's answer did not depend on, or an engine value away from exact inference by more than the tolerance — and nothing in any rendering said so unless the duty's own verdict happened to carry it into the summary. `report.certificate_findings` exposes those measurements as findings and all three renderings show them where they belong to the requirement: one `certificate finding: FAIL (decision N)` line in the text report, a callout in the HTML, and a `findings` list on each result in `--json` (`results[].findings`, beside `details` rather than inside it). **It is a measurement and not a verdict**: the duty's verdict, strength, basis and witness are untouched, `JSON_SCHEMA_VERSION` is unmoved because the key is additive, and every audience projection that withholds the evidence summary withholds these too. The committed transcripts, the showcase cast and `docs/showcase.html` are regenerated to show the line. ([#157](https://github.com/eduardstan/reasonsmith/pull/157))
...
- **The JSON envelope carries the undeclared-domain notice.** `ConformanceReport.to_dict()` now
  ...
  an audience-filtered finding set. ([#155](https://github.com/eduardstan/reasonsmith/pull/155))

- **The inference artefact admits a perturbation beyond deletion, and the refusal that forbade one is reversed on the record.**
  `artifacts.InferenceArtifact` stated that there is a `without(fact)` and deliberately no `with_(fact)`, and [`docs/theory/07-explanation.md`](docs/theory/07-explanation.md) §7.1 stated the matching limit; a family may now also offer `at(fact, probability)` and `probability(fact)`, read through `artifacts.admits_interpretation`. **The reversal is recorded rather than performed silently** — in the module docstring, and in §7.1 in that chapter's own notation — in the shape [`ROADMAP.md`](ROADMAP.md) §2 records the `since` reversal: what was refused, what changed, and what is still refused. What was refused was written for the *deletion* definition of a reason and its ground is the soundness of `deleted`. What changed is that a measurement was designed that never touches `deleted` and does need the width. What is still refused is everything that ground was protecting: no definition in §3 quantifies over anything but the deletion lattice `L(β)`, `certificate.py` and `explanations.py` call `without` and nothing else — checked, not asserted, by `test_the_deletion_probe_never_reaches_the_widened_perturbation` — and a family offering no `at` loses no verdict, because no requirement in this repository reads the wider surface. `artifacts.reason_trace` deliberately offers none: a rationale the system recounted has no interpretation to move. **No shipped verdict moved**, and the deletion probe's own numbers are byte-identical, `without(fact)` now being `at(fact, 0.0)`.

  ([#158](https://github.com/eduardstan/reasonsmith/pull/158))

- **`src/reasonsmith/semantic_laws.py`: a false `claimed_semantics` refuted from the system's own answers, with no reference implementation in the loop.**
  An artefact's `claimed_semantics` is printed on the certificate and checked by nothing. The claim is universally quantified over interpretations, so nothing on this evidence model establishes it — but one witness refutes it, and this measures that. Two laws are checked per fact, both consequences of `⟦P, β, q⟧ = Pr_β[φ_q]`: multilinearity (Shannon expansion, which needs no premise about the enumeration being complete) and monotonicity (which needs only the positivity of the supports §3.1 already uses). [`docs/theory/07-explanation.md`](docs/theory/07-explanation.md) §7.7 states both with the soundness proposition, the three things it does not claim, and why the design's third law — the vertex law — is deliberately absent: its premise is one a depth-bounded artefact cannot establish, and a law whose premise the instrument cannot check is a false-accusation machine. Over the 16 generated instances `docs/build_nesyarena_report.py` drives, every one of the four `nesyarena` provenances that does not implement the distribution semantics it claims is refuted (4/16, 8/16, 4/16, 12/16) and the one that does implement it is refuted on none; the `nesyarena` oracle is consulted only to populate the column the run is compared against and plays no part in any law. This is a **measurement and not a verdict**: it returns no `RequirementResult`, occupies no rung, is read by no requirement and is deliberately not under `engines/`, the standing `ltlf.py` has. Refutation is a lower bound on deviation and `LIMITS` says so — one provenance deviates on 16 instances and is refuted on 12. Two vocabularies are kept apart: `spec.CLAIMED_SEMANTICS` is what an artefact may claim, closed and refused outside itself at the artefact and certificate boundaries, and `semantic_laws.SEMANTICS_WITH_LAWS` is the one member of it this tool has laws for — derived by intersection rather than retyped. An admitted claim with no law is *not evaluated* naming the claim; a claim outside the vocabulary reaches no law at all, because the boundary refuses it first. The width is what buys the discrimination and it was measured: neither one-directional variant of the triple refutes a top-`k` engine on any of the 16 instances, while the spanning triple refutes `top-1-proofs` on 8 (`test_neither_one_directional_variant_refutes_a_top_k_engine`), which is the evidence the reversal above rests on.

  ([#158](https://github.com/eduardstan/reasonsmith/pull/158))

- **When the counterfactual duty's two rungs disagree, the report names the cause rather than the symptom.**
  The `proved` and `probed` rungs of `counterfactually_invariant(o, p)` do not range over the same object: the first asks whether the *declared rules* treat alike every pair the *declared constraints* admit, the second whether the *implementation* treated alike the pairs built from the decisions the system logged. So their disagreement is evidence in its own right, and [`docs/theory/04-decision-problems.md`](docs/theory/04-decision-problems.md) §4.4 now states the relation — `R ⊆ P` and `decide_S` agreeing with the declared logic on the declared space together make `probed = violated` imply `proved = violated` — with the contrapositive that is the whole feature: `proved = satisfied` beside `probed = violated` eliminates one of those two hypotheses. The lower rung is therefore run whenever the higher one reached a verdict, and a disagreement is reported as the **disjunct it eliminates** under `engines.counterfactual.RUNG_DISAGREEMENT_KEY`. `proved = violated` with `probed = satisfied` is the relation holding rather than a defect, and what it names is the log: it does not exercise what the rules permit. The other direction discharges the cheap hypothesis first, by evaluating the declared constraints on the replayed pair — a pair the declared input space does not admit is named as such and the declaration is not impeached, and only a pair *inside* that space leaves the residual finding, that the system's `decide()` does not implement the `logic()` it declared. A record that leaves a declared constraint unsettled eliminates neither and says so. The disagreement annotation itself moves no verdict, strength or witness; the paired-replay completeness change recorded above is separate and can replace a partial `probed` satisfaction with *not evaluated*. No shipped system reaches either rung on the one shipped counterfactual duty, so no shipped verdict moved. `tests/test_counterfactual_invariance.py` carries a witness per direction, including the second one the item allowed might be unreachable — it is reachable, and the fixture is a rule set whose `decide()` deliberately departs from what it declares. No new shipped example system, and no shipped record holding a protected attribute about a natural person.

  ([#147](https://github.com/eduardstan/reasonsmith/pull/147))

- **The proposition the finite-trace decision procedure rests on is in the mathematics document, with its proof.**
  0.8.0 re-encoded `ltlf.accepts(φ, σ)` as satisfiability over the characteristic formula `pin(σ)`, and the soundness of that move — `L(pin(σ)) = {σ}`, without which the solver is answering about a set of traces while the tool reports an answer about the one it was given — was stated only in `src/reasonsmith/ltlf.py`'s own docstring. [`docs/theory/05-decision-procedures.md`](docs/theory/05-decision-procedures.md) §5.6 now states and proves it in the document's notation, with the two corollaries the production surface uses, and records three things the proposition depends on that were previously implicit: why the completeness of each `λᵢ` over `AP` is load-bearing, that the non-emptiness the proof assumes comes from BLACK's own finite-trace semantics rather than from the `NON_EMPTY` guard 0.8.0 deleted (and the one reachable shape — an atom-free formula — where it bites), and that `ATOM_BUDGET` counts *distinct* atoms, which `pin(σ)` adds none of, so the encoding's growth with the trace length is in literal occurrences and not in the count the budget refuses on. `[@biere-1999]` and `[@geatti-2021]` join the bibliography as the bounded-model-checking move and the finite-trace interpretation the section relies on. `test_pin_characteristic_formula_accepts_sigma_and_rejects_neighbors` now asks the solver about **every** trace one Hamming step from σ rather than two of the four, and no verdict, engine or rung moved.

  ([#144](https://github.com/eduardstan/reasonsmith/pull/144))

- **`reasonsmith explain <requirement-id>` shows how a clause of law became a formula.**
  Every requirement already carried its clause, its formula, its rationale and the signals it needs as fields of the pack, and none of it reached a reader through the tool: `validate-pack` prints one index line per requirement, so someone asking how a clause became a property had to open the pack TOML, [`docs/refinement.md`](docs/refinement.md) and [`docs/theory/02-syntax.md`](docs/theory/02-syntax.md) and hold three files in their head. The command prints those fields under `CLAUSE`, `FORMULA`, `RATIONALE`, `FRAGMENT` and `REQUIRES`, and adds `REFINEMENT` — the fourth column of the refinement record, what the formalisation deliberately did not capture. It runs no engine and reads no system, so it changes no verdict. The id resolves against every built-in pack, or against the packs `--pack` names, a TOML path included; an id nothing ships is a usage error naming the packs searched, never an empty frame. `docs/` is not in the wheel, so an absent refinement record is *named* and pointed at the repository rather than silently dropped. No rung ceiling is printed: which rung a duty reaches is decided at run time by whichever engine serves it, not by its fragment, and a table of fragment-to-rung here would be a hand-maintained claim nothing holds to the dispatch. `tests/test_explain_command.py` explains every shipped requirement and holds both halves of the refinement section; `tests/test_docs_refinement.py` now reads the record through the same parser the command uses, rather than keeping a second copy.

  ([#143](https://github.com/eduardstan/reasonsmith/pull/143))

- **[`docs/adopting.md`](docs/adopting.md): the document for a reader who arrives with a system of their own.**
  There was an authoring guide for someone writing duties and one for someone writing an engine, and none for the adopter, whose question is "here is my system — what can you tell me about it?". [`docs/three-systems.md`](docs/three-systems.md) walks three systems, but they are *shipped example* systems and it demonstrates the evidence ladder rather than being an on-ramp. The new page arranges what the repository already documents for that reader: the four surfaces a system can come in through and the honest ceiling of each, the pairing of `reasonsmith explain` (which duty needs which signals, and what its formalisation left out) with `report.analyze_unattainable` (what happens when the system does not emit them), and a worked run on a log a notice service could plausibly export — four duties reported unattainable and one not evaluated, before one added field turns an unanswerable duty into a reported violation and the exit code from 0 to 2. It states the limit first rather than last: every verdict rests on declarations nothing here checks, measured by finding 1 of [`docs/findings-nesyarena.md`](docs/findings-nesyarena.md); objective 5 of [`ROADMAP.md`](ROADMAP.md) now closes that gap for one artefact family and states the declarations and families it still cannot reach. It cites [`docs/semantics.md`](docs/semantics.md) §4 for the four outcomes rather than keeping a second copy of that table. `tests/test_docs_adopting.py` re-runs every command on the page and compares stdout byte-for-byte, and holds §2's set-difference claim to `analyze_unattainable` itself, including that it answers without executing the system.

  ([#145](https://github.com/eduardstan/reasonsmith/pull/145))

- **The repository states how to report a vulnerability, how long a contribution waits, and what a commit message may not carry.**
  There was no `SECURITY.md`, so a reporter with a finding had a public issue tracker and nothing else. The new top-level file names a private reporting address, the acknowledgement and assessment windows, and the supported version, and it is explicit about **scope** in the one place this tool is easy to misread: reasonsmith shells out to the BLACK LTLf solver, loads TOML packs and imports a caller-supplied system module, none of which it sandboxes — so an intentionally malicious pack or system module run with the operator's own permissions is not by itself a vulnerability here, while anything that unexpectedly executes code, escapes the process boundary, exposes data or misrepresents a security-relevant result is. `CONTRIBUTING.md` gains the matching *Response times* windows and an *Attribution convention*: no AI co-author trailer or AI-attribution footer on a commit, pull-request body or comment, human co-author trailers welcome, stated as a contributor-facing convention with no check enforcing it. Nothing in the package moved. ([#159](https://github.com/eduardstan/reasonsmith/pull/159))
...
- **The coverage gate no longer trips Hypothesis's per-example deadline under instrumentation on a busy machine.**
  ...
  ([#146](https://github.com/eduardstan/reasonsmith/pull/146))

- **Coverage headroom, bought with tests for behaviour that had none.**
  355 lines of tests across six modules — `test_autoformalize.py`, `test_probed_engine.py`, `test_evidence_basis.py`, `test_open_texture.py`, `test_v02_core.py` and `test_v02_stage2.py` — reach paths the suite had been leaving to the 92.5 floor's margin rather than to a test. No production line changed with them, so no verdict, engine, rung or document moved; what changed is that the release's own floor has room above it that is evidence rather than luck. ([#153](https://github.com/eduardstan/reasonsmith/pull/153))

- **A push to a branch with an open pull request no longer runs the test matrix twice.**
  `.github/workflows/ci.yml` fired on every `push` and every `pull_request`, so one push to a PR branch queued two identical matrices — twelve checks where six are the same evidence. The `push` trigger is now limited to `main`, and a `concurrency` group keyed on workflow and ref cancels a superseded branch run while never cancelling one on `main`. This is a repository-infrastructure change with nothing in it for someone who installed the package: no engine, verdict, pack, rung, document or command moved, and the same matrix, the same suite and the same 92.5 coverage floor still gate every pull request. It is recorded here because the account of this release is the whole `v0.8.0..HEAD` delta, not the product-facing part of it. ([#163](https://github.com/eduardstan/reasonsmith/pull/163))
...
- **§1.7's exhaustiveness claim is narrowed to the scope its pin has.**
  [`docs/theory/02-syntax.md`](docs/theory/02-syntax.md) Definition 2.5 records the grammar refusals. At least five have none, and three of them are not defensive branches: `classify_fragment` accepts the specification and assigns it a fragment, and the refusal fires later, at evaluation, for `counterfactually_invariant()`, `undetermined()` and `degree()`. The claim now states what the pin actually holds — the refusals reachable from `classify_fragment`, which is the entry point every witness runs through — names the three evaluation-time refusals as outside the table's scope, and points at `docs/theory/03-semantics.md` §3.12 and `docs/theory/08-evidence.md` §8.4 where each is documented. No row was added for them: a witness for one would have to run through an engine, so a row would be a rule the pin cannot check, which is the hollow-pin failure this release already had to repair once.
...
- **The mathematics is stated once, and the repository has a bibliography that the build enforces.**
  The numbered theory chapters now gather the mathematics that was scattered across four documents
  and a dozen module docstrings into one notation: the objects, denotation, sufficient-reason
  definitions, deletion certificate, evidence coordinates, residuated lattices, and engine procedures.
  They introduce no construct, engine, rung, basis or verdict. The repository-wide bibliography lives in
  `docs/theory/bibliography.md` and remains a registry: a citation is a backticked pandoc key, and
  `tests/test_docs_formal.py` fails the build when a key resolves to no entry, when an entry is cited by
  no claim, or when a paragraph anywhere in `docs/` or `src/reasonsmith/` names a publication venue
  and carries no key. The anti-drift mechanism for definitions is generated from the code in each
  chapter that states them.
...
- **The strength lattice gains a rung, and the inference-artefact protocol gains a second family.**
  ...
  fifth basis, by the test [`docs/theory/08-evidence.md`](docs/theory/08-evidence.md) §8.2 now states: evidence about a
...
- **The property language has a formal semantics, and it is a checked one.**
  ...
  [`docs/theory/02-syntax.md`](docs/theory/02-syntax.md) defines it: an EBNF grammar checked against the parser
...
- **Reported: four shapes on which the trace-rung implementation and the definition disagree.**
  ...
  **latent** — no shipped pack writes any of them, and a test now keeps that true. rtamt's lexer has
  ...
  nor a landed fix leaves the list stale. See `docs/theory/02-syntax.md`.
...
- **The evidence scale gained a second coordinate, and the strength lattice did not move.**
  ...
  rather than after this repository (`docs/theory/08-evidence.md` §8.2 carries the citations). Four things are
...
- **`validate-pack --analyse` has a decision procedure for temporal duties, beside rtamt rather
  ...
  from a weaker substitute. Four costs are stated in `docs/theory/05-decision-procedures.md` §5.4 rather than left to be
...
- **A pack can be checked against itself: `reasonsmith validate-pack <pack> --analyse`.**
  ...
  `docs/theory/04-decision-problems.md` §4.6; and, with `--system-module`, a **mutation score per duty** over
...
- **An inference artefact is reasonsmith's own abstraction, and a ground program is one adapter.**
  ...
  it needs an adapter (`docs/theory/07-explanation.md` §7.1, *The inference artefact*).
...
- **The deletion probe states its direction, and keeps the one fingerprint of an engine that
  breaks it.** `certificate.LIMITS` and `docs/theory/07-explanation.md` §7.5 (*certificate*) now say that the
...
- Nothing shipped changes verdict. A log that states no clock is `"ordinal"` and is answered on the
  ...
  (`docs/theory/03-semantics.md` §3.5, `docs/semantics.md` §4; `tests/test_time_domain.py`).
...
- **Breaking (verdicts move): a duty whose trigger never fired is reported *not evaluated*, at
  ...
  `docs/theory/05-decision-procedures.md` §5.1 and `docs/semantics.md` §1 state it per rung; `tests/test_vacuous_trigger.py` holds it. The
...
- **Breaking:** **An adapter declares which variables the system computes, and the proof engine
  ...
  `docs/theory/05-decision-procedures.md` §5.2, *When the magnitudes are not the system's own*, is rewritten around the
...
- **A decision whose reasons bounded proof enumeration never found buys no verdict, and never
  ...
  still reaches `satisfied` at `probed`. `docs/theory/07-explanation.md` §7.5's certificate guarantee now says
...
- **A declaration of what a system computes can no longer widen what it can be proved about**
  ...
  *instead of* `_check_magnitudes_are_computed` wherever `sut.logic()` declared `computes`, and it
  ...
  assertion about itself that no rendering names — the self-declaration `docs/theory/05-decision-procedures.md` §5.2
...
- **The affected-individual report is derived for its reader, not the expert report with parts
  ...
  document regenerates byte-for-byte. `docs/semantics.md` §3 loses the limit it used to state and
...
- **A temporal duty reaches `proved`, where its shape reduces to a property of one decision**
  ...
  *replaced*, never deleted, by `test_only_always_reaches_the_temporal_proof_rung`, which pins the
  new ceiling from both sides; the soundness paragraph is `docs/theory/05-decision-procedures.md` §5.1, *reference interpreter*. `examples/symbolic_rules.py` gains the notification and margin rules the two shipped
...
- **The front page shows the audience view and says the tool is extensible**
  ...
  `semantics.md` now names §3 so the audience table is findable from the index. No behaviour
...
- **An engine and a pack can be installed rather than vendored**
  ...
  *can reasonsmith use Prolog, ASP, or a different solver?* is now **yes, as a package you install**,
  ...
  `test_with_no_plugin_installed_the_ladder_is_the_builtin_ladder`; `docs/theory/05-decision-procedures.md` §5.2 carries
...
- **One run, five artefacts: `reasonsmith check --audience {developer,deployer,auditor,regulator,affected-individual}`.**
  ...
  derived and is written down in `docs/semantics.md` §3, along with two limits — this artefact
...
- **The reason-deletion certificate is an engine.** `certificate.py` could measure which reasons an
  ...
  *inputs* to `certificate.certify` for the inference a decision came from (never a verdict — a
  ...
  2. See [`docs/theory/07-explanation.md`](docs/theory/07-explanation.md) §7.5, *certificate*.
...
- `contains(signal, "phrase")`, a property-language atom that reads *what a statement says* rather
  ...
  [`docs/theory/02-syntax.md`](docs/theory/02-syntax.md) and [docs/authoring-packs.md](docs/authoring-packs.md),
...
- **A `logical` duty is now answered from a decision trace.** A `logical` property is a property of
  ...
  could be checked. `docs/theory/05-decision-procedures.md` §5.2 already stated the principle in its first bullet and
...
- **12 CFR 1002.9(b)(2) checks the clause's own negative constraint, and carries its trigger.** The
  ...
  ([docs/semantics.md](docs/semantics.md) §1).
  ([#64](https://github.com/eduardstan/reasonsmith/pull/64))
...
- A second applicability gate, `domains`, beside the existing regulatory-class `scope` gate. It
  ...
  [docs/semantics.md](docs/semantics.md) §1 for what a not-applicable verdict on it does and does
