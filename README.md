...
It exits 2 when a requirement is violated or unattainable, 0 otherwise. There is no report renderer beyond text/JSON. Capabilities the CLI did not receive explicitly are read from the supplied log, and a result that rests on that says so rather than speaking for the system.
