...
### Dependencies & PyPI
- **`nesyarena`**: Supplies the ground-program IR, bounded proof enumeration, exact oracle, and adapter protocol. It is pinned to an immutable git commit in `pyproject.toml` so measurements stay reconstructible. Availability fallback: the pinned commit is archived on Software Heritage ([`swh:1:rev:57720fa212834689692e171882272140f1d1fed7`](https://archive.softwareheritage.org/swh:1:rev:57720fa212834689692e171882272140f1d1fed7)), so the identical tree stays fetchable even if the GitHub remote moves. Whether `nesyarena` is published to PyPI is its author's decision; the pin stays until then.
