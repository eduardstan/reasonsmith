...
Install the optional terminal interface with:

```sh
pip install "reasonsmith[tui]"
```

Then explore a conformance report and its evidence graph:
...
The TUI is an interactive presentation of the same `ConformanceReport` used by the text, JSON,
and HTML renderers. It keeps `satisfied`, `violated`, `inconclusive`, `not evaluated`,
`unattainable`, and `not applicable` distinct, and keeps the report's limits visible. Use `j`/`k`
to inspect requirements, `a` to cycle audience projections, `r` to refresh, and `q` to quit.

![reasonsmith TUI evidence explorer](docs/assets/tui.svg)

The SVG above is a terminal-native visualization generated by `docs/build_tui_visualization.py`;
it is intended for demos and review, not as a substitute for the interactive application.
