...
- **Developer:** Start with [`docs/adopting.md`](docs/adopting.md), follow the theory spine from [`docs/README.md`](docs/README.md), and finish at [`docs/authoring-engines.md`](docs/authoring-engines.md). Runnable demos: [`notebooks/`](notebooks/) ([reader index](notebooks/README.md); quickstart, evidence rungs, and contributor scaffolds).
...
*The terminal interface is not part of the Python distribution and requires Bun.* [Install and launch it](packages/tui/README.md) · [Watch the recording](artifacts/tui/tui-check/demo.mp4) · [View the layout report](docs/tui/layout-report.png)
...
The full destination map is [`docs/README.md`](docs/README.md); the live dossier is [reasonsmith.dev/report.html](https://reasonsmith.dev/report.html); empirical measurements are in [`RESULTS.md`](RESULTS.md), the backlog in [`ROADMAP.md`](ROADMAP.md), and contribution rules in [`CONTRIBUTING.md`](CONTRIBUTING.md). Contributor governance and compensation commitments are in [`GOVERNANCE.md`](GOVERNANCE.md).
