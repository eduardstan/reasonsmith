...
does change is the suite: section 2's `35 passed` counts it as it stood then, and the v0.2 work
added since (`verdict.py`, `spec.py`, `sut.py`, `report.py`, `adapters/`, `engines/`, `cli.py`, the
`packs/` requirement packs, and `tests/test_v02_core.py` plus `tests/test_v02_stage2.py`) adds tests
