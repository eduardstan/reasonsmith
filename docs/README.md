...
| [`adapters/`](adapters/) | The four adapter files `three-systems.md` and `language-model.md` walk through (`neural_scorer.py`, `probabilistic_scorer.py`, `symbolic_rules.py`, `language_model_notices.py`): complete, runnable systems, each on its own with `python docs/adapters/<name>.py`. |
