...
*fields* of a requirement. This document is the record of the *judgement*: for each of the 41 shipped
...
clear the model-free round-trip and all of its gold cases before a human reviews it. The explicit
`Human sign-off:` marker on each row below is the third gate; `pending` means no candidate is
...
every emitted report, and [`docs/semantics.md`](semantics.md) §4 states it. The distance between the formula and the
...
- **Presence is not adequacy.** Twenty-nine of the forty-one shipped duties are `record` duties:
  ...
  **A third escape now exists as machinery, and no row below uses it.** `undetermined(signal,
  ...
  [`docs/theory/08-evidence.md`](theory/08-evidence.md) §8.5 is the semantics, `docs/authoring-packs.md` is how to write one, and
...
- **The trace is a sample.** An `observed` or `record` verdict covers exactly the records supplied,
  ...
  ([`docs/semantics.md`](semantics.md) §4). Any clause whose duty runs over the lifetime of a system —
  ...
  trace that system can emit rather than the sampled one ([`docs/theory/05-decision-procedures.md`](theory/05-decision-procedures.md) §§5.1–5.2, *`proved`, over a
...
- **Organisational facts are outside every engine.** A controller's legal basis, a staffed appeals
  ...
  says, not whether it was honest ([`docs/semantics.md`](semantics.md) §4, *operational boundaries*).
...
every duty that limits either, never `satisfied`, and reasonsmith infers neither ([`docs/semantics.md`](semantics.md) §1). The Seoul pack adds a third, pack-level applicability gate: its adapter-declarable
...
- **What the system declares is a self-declaration.** Nothing checks that a system that says
  ...
  ([`docs/semantics.md`](semantics.md) §4, *operational boundaries*). The gate stops a duty reaching a system
...
- **The trigger inside a decision is still not modelled *as a gate*, and that is the larger
  ...
  ([`docs/semantics.md`](semantics.md) §1). That is the truthful report of a duty nothing was learned about, and it
...
rebuilding anything on that premise it is worth knowing how many of these forty-one entries the
...
**The count, over all forty-one shipped requirements:**

| `defeasibility` | Requirements | What it says |
|---|---|---|
| `defeasible-unmodelled` | **9** | The law states an exception; the property does not carry it. |
...
The nine are `ecoa_reg_b_1002_9_a_1_timing_of_notice` (paragraph (c)'s incomplete-application
...
properties, and the two Article 50 rows, whose paragraph (1) and (2) exceptions remain outside their properties, and `eu_ai_act_art86_1_main_elements_of_the_decision`, whose clause carries **one defeater**
...
|---|---|---|---|
| GDPR Article 22(1)<br>`gdpr_art22_1_automated_decision_prohibition` | A person has the right not to be subject to a decision based solely on automated processing that produces legal or similarly significant effects. | `record`: `present(artifact_logs_decision_record) and present(provenance_active_exceptions)` | The prohibition itself. This property witnesses that the system keeps a decision record and records which exceptions were active — it is a logging duty standing in for a substantive one. A system that makes exactly the decision Article 22(1) protects against satisfies it, provided it logs that decision. Nothing here witnesses the data subject's right, its exercise, or the controller's response. The prohibition is carried by the next row, and only against a system that exposes its rules. | Human sign-off: pending (gold set: `gdpr_art22_1_automated_decision_prohibition.toml`; no candidate approved).
| GDPR Article 22(1)<br>`gdpr_art22_1_no_prohibited_decision_for_any_input` | Same clause, read as a constraint on the decision rules: no admissible input may yield a solely-automated, significantly-affecting decision without an Article 22(2) basis, and under points (a) and (c) without the Article 22(3) route open. | `logical`: `(artifact_logs_solely_automated and artifact_logs_significant_effect) -> ((provenance_basis_contract or provenance_basis_union_or_member_state_law or provenance_basis_explicit_consent) and ((provenance_basis_contract or provenance_basis_explicit_consent) -> artifact_logs_human_intervention_route))` | Whether any of those flags is true of the world. Each Article 22(2) basis is a legal fact about the controller — a contract that is necessary, a Union or Member State law that authorises, consent that is explicit and freely given — and the property reads a Boolean the system sets about itself. The pack description says so; [`docs/theory/05-decision-procedures.md`](theory/05-decision-procedures.md) §5.2 (*proved*) states that a proof is a claim about the logic exposed through `logic()`, not about the deployed artifact. Also not captured: Article 22(3)'s *suitable measures* collapse to one flag, so a route that is unstaffed, untimely or powerless to change the decision reads the same as a real one; Article 22(2)(b)'s requirement that the authorising law itself lay down safeguards is not modelled; Article 22(4)'s restriction on special-category data is not formalised anywhere in this pack. The implication is spelled with `->` rather than the equivalent `Implies(...)`: the two are the same property to every engine that parses the spec, and only the arrow is renderable by the rtamt monitor that now reads this duty off a trace, so the prefix form would leave it not evaluated against a log for a reason having nothing to do with the clause ([`docs/theory/05-decision-procedures.md`](theory/05-decision-procedures.md) §5.1). | Human sign-off: pending (gold set: `gdpr_art22_1_no_prohibited_decision_for_any_input.toml`; no candidate approved).
| GDPR Article 22(3)<br>`gdpr_art22_3_safeguards_human_intervention` | The controller must implement suitable measures safeguarding the data subject — at least human intervention, expressing a point of view, and contesting the decision. | `record`: `present(artifact_logs_decision_record) and present(scope_statements_local_vs_global)` | All three named rights. Neither conjunct witnesses human intervention, a point of view or a contest; `scope_statements_local_vs_global` is a statement of explanation scope, chosen by the pack author as a proxy for *the safeguard can say what this decision rested on*. That proxy is editorial and the clause does not name it. *Suitable* is an adequacy judgement no engine here makes. And the clause is limited to cases under points (a) and (c) of Article 22(2), while the property is checked against every record in the trace — its reach is wider than the clause's. | Human sign-off: pending (gold set: `gdpr_art22_3_safeguards_human_intervention.toml`; no candidate approved).
| GDPR Recital 71<br>`gdpr_recital71_meaningful_explanation` | Suitable safeguards should include specific information, human intervention, a point of view, an explanation of the decision reached, and a challenge to it. | `record`: `present(artifact_logs_reason_explanation) and present(scope_statements_explanation_scope) and present(provenance_model_version)` | *Meaningful*, which is the entire content of the recital. Presence sees a non-empty string; `"n/a"` is present ([`docs/theory/03-semantics.md`](theory/03-semantics.md) §3.5, *record*). Nothing checks that the explanation is about the decision it accompanies, that a lay reader could act on it, or that the model version named is the one that ran. A recital creates no obligation of its own, which is what `binding = false` records — the row is an interpretive reading, not a duty. As above, the reach is every record rather than only Article 22 processing. | Human sign-off: pending (gold set: `gdpr_recital71_meaningful_explanation.toml`; no candidate approved).
| GDPR Recital 71<br>`gdpr_recital71_error_risk_minimised` | The controller should use appropriate procedures and measures so that inaccuracies are corrected, the risk of errors is minimised, data is secured, and discriminatory effects on the listed protected grounds are prevented. | `temporal`: `always(artifact_logs_semantics_value_gap <= artifact_logs_decision_margin)` | Every limb but one. The property formalises *the risk of errors is minimised* and nothing else: correcting inaccuracies in personal data, security, and the prevention of discriminatory effects on racial or ethnic origin, political opinion, religion, trade union membership, genetic or health status or sexual orientation are not formalised here or anywhere in the shipped packs — **no *distributional* fairness property is checked by any requirement in this repository; the one fairness property that is checked is counterfactual invariance under a single named variable (`ecoa_reg_b_1002_4_a_no_disparate_treatment`), and it cannot see a disparate impact.** That duty is anchored to a disparate-*treatment* clause of Regulation B and not to this recital, for the reason `docs/legal-sources.md` records under Provision 4: the recital's own words are *discriminatory effects*, and effects is the limb a property of a pair of decisions cannot reach. What the error limb itself does and does not claim is stated in full in `docs/theory/07-explanation.md` §7.5 (*the certificate and value gap*). Since the left-hand side became a measurement, the fourth column owes a second entry, because what the reading captures and what it leaves out both changed. **What it now captures:** the error is measured by reasonsmith from the inference artefact behind the decision — exact inference over the model encoding against the answer the system's own engine gave — so a system that under-reports its own approximation no longer passes on the strength of the report, and two systems alike but for whether their inference implements the semantics they declare no longer produce the same conformance report. **What it still does not capture, and this is narrow:** one artefact family. The measurement needs a declarative model encoding exposed through `artifact()` whose exact side computes the semantics the system claims; a log-only system, a language model behind `complete()`, a recounted reason trace and the five `nesyarena` provenances as adapted in `docs/build_nesyarena_report.py` are all `unattainable` on it, and that is the reach honestly stated rather than a gap in the pack. **Nor does it capture a documented approximation.** `reasonsmith.spec.CLAIMED_SEMANTICS` is closed and has no member for an approximation *of* another member, so a system that honestly documents its truncation cannot declare the one semantics this build computes a reference for; the duty reports it *not evaluated* naming the claim rather than comparing it against a semantics it never made. That is deliberate — the alternative is accusing an honest system — and it means the duty is silent about exactly the population most likely to have a large error. Where the artefact exposes a finite decision threshold, the bound is the measured distance between the system's answer and that threshold; where it does not, the system's declared record margin remains the fallback. Neither is a figure the recital states, and an exact tie is still reported satisfied, the clause's own comparison being non-strict. A malformed threshold, non-finite inference value, or non-finite derived margin is not evaluated rather than guessed. The existing `artifact_logs_decision_margin` capability remains a conjunctive reach gate. | Human sign-off: pending (gold set: `gdpr_recital71_error_risk_minimised.toml`; no candidate approved).

## EU AI Act (Regulation (EU) 2024/1689) — `src/reasonsmith/packs/eu_ai_act.toml`

The four original duties and Article 86(1) carry `scope = "high-risk"`; the two Article 50 duties carry `scope = "limited-risk"`. A system that declares no class is reported
`not_applicable` on all seven, and reasonsmith never infers the class ([`docs/semantics.md`](semantics.md) §1).

| The clause | The informal duty | The formal property | What was deliberately not captured |
|---|---|---|---|
| EU AI Act Article 12(1)<br>`eu_ai_act_art12_1_automatic_logging` | A high-risk system must technically allow automatic recording of events over the system's lifetime. | `record`: `present(artifact_logs_event_log) and present(provenance_model_version)` | *Over the lifetime of the system*, and *automatic*. The check reads the trace supplied for one run — a sample chosen by whoever produced it ([`docs/semantics.md`](semantics.md) §4) — so retention period, log durability and whether records survive a model update are all outside it. A log hand-assembled for the audit is indistinguishable from an automatically recorded one. Article 12(3)'s minimum content for the logs, and Article 19's retention obligation, are not formalised. This property is byte-identical to the Article 12(2) property below, and the next row records why the two are not told apart. | Human sign-off: pending (gold set: `eu_ai_act_art12_1_automatic_logging.toml`; no candidate approved).
| EU AI Act Article 12(2)<br>`eu_ai_act_art12_2_traceability_monitoring` | Logging must enable recording of events relevant to identifying risk situations and substantial modifications, to post-market monitoring, and to Article 26(5) operation monitoring. | `record`: `present(artifact_logs_event_log) and present(provenance_model_version)` | The three limbs, individually and together. The property is byte-identical to the Article 12(1) property, so **no system can satisfy one of these two duties and violate the other** — they cannot come apart in any report, and a reader must not take two agreeing verdicts for two independent checks. *Appropriate to the intended purpose* is an adequacy judgement nothing here makes. The overlap is **recorded rather than repaired, and this is the reasoning.** What separates the two clauses is *relevance*: 12(1) asks that events be recorded at all, 12(2) that the events recorded be the ones relevant to identifying a risk situation or a substantial modification, to post-market monitoring under Article 72, and to Article 26(5) operation monitoring. Nothing in the Section 6.3 signal vocabulary (`sut.CAPABILITY_TAXONOMY`) stands for the relevance of a logged event to any of those three ends, and no per-decision record witnesses it — relevance is a property of a logging *design* judged against an intended purpose, not a field a decision carries. A signal minted for it here would be this pack author's reading of the Article shipped as the Article's, the same objection `docs/authoring-packs.md` (*a number in a spec*) makes to an invented threshold, and it would report every system that does not declare the invented signal unattainable on 12(2) — trading a visible overlap for an invisible one. The pack description carries this so a reader of the tool's own output meets it too. The overlap is also no longer a fact only this table knows: `reasonsmith validate-pack eu_ai_act --analyse` reports the two properties equivalent without anyone reading either `[[requirement]]` block ([`docs/theory/04-decision-problems.md`](theory/04-decision-problems.md) §4.6). | Human sign-off: pending (gold set: `eu_ai_act_art12_2_traceability_monitoring.toml`; no candidate approved).
...
| EU AI Act Article 50(5)<br>`eu_ai_act_art50_5_disclosure_timing` | At the latest at the first interaction or exposure, provide the Article 50(1)–(4) information to the natural person. | `temporal`: `always(present(artifact_logs_natural_person_interaction) -> present(artifact_logs_ai_disclosure))` | This pilot admits the temporal property only over a versioned, complete execution record whose interaction and disclosure events share a correlation identifier and carry explicit event-clock entries plus boundary-recorder provenance. A non-blank `artifact_logs_ai_disclosure` or `disclosure_delivered` field from the agent is refused, not observed evidence; an omitted event is likewise inconclusive. The boundary event pair still does not establish that a person saw the disclosure, or that the information was clear, distinguishable, accessible, understandable, or actually understood. It does not decide whether the system is intended to interact directly with natural persons: applicability remains a declared input. The exceptions in Article 50(1) (obvious interaction and authorised criminal-offence systems) and the scope of paragraphs 1–4 are not modelled; nor are paragraphs 50(3) and 50(4). The trace is the supplied synthetic sample, not the system’s lifetime. | Human sign-off: pending (gold set: `eu_ai_act_art50_5_disclosure_timing.toml`; no candidate approved).
| EU AI Act Article 50(2)<br>`eu_ai_act_art50_2_synthetic_content_marking` | Mark synthetic audio, image, video or text outputs in a machine-readable format so they are detectable as artificially generated or manipulated. | `record`: `present(artifact_logs_synthetic_content_mark)` | Presence only: a non-empty marking field is not proof that the marking is machine-readable, detectable, effective, interoperable, robust, reliable, or technically feasible, and it says nothing about the state of the art or implementation costs. This check does not establish that the output is synthetic content, that the marking is attached to that output, or that any Article 50(2) exception applies: assistive standard editing, no substantial alteration, and legally authorised criminal-offence uses are deliberately unmodelled. The provider's role and the system's general-purpose status are not inferred. | Human sign-off: pending (gold set: `eu_ai_act_art50_2_synthetic_content_marking.toml`; no candidate approved).
...
## Cyber Resilience Act (Regulation (EU) 2024/2847) — `src/reasonsmith/packs/cra.toml`

One duty, and it is the first shipped property written on the event clock rather than on the
record index. Article 14(2)(a) states an explicit outer bound, which is what makes it
formalisable at all: the metric layer measures the elapsed time between two independently
timestamped events and decides nothing about the clause's open-textured limb. The pack quotes
`docs/legal-sources.md`, and the drift check re-fetches the Official Journal text.
