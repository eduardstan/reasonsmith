...
- **Presence is not adequacy.** Fourteen of the eighteen shipped duties are `record` duties: a
  conjunction of `present(signal)` atoms, and a fifteenth is the same presence check quantified
...
- **The property's reach is not the clause's scope.** Most clauses below are triggered — by adverse
  ...
  and the decision domain (`domains`, used by four) — and no trigger *inside* a decision is.
...
- **The trigger inside a decision is still not modelled, and that is the larger remaining gap.**
  12 CFR 1002.9 is triggered by *adverse action having been taken*, not by the creditor being in
  consumer credit. A domain declaration puts the whole trace within the clause's subject matter and
  the property is then evaluated over every record in it, approvals included. Every "the clause is
  triggered, the property is not" note in column four below is untouched by this gate — see the
  (a)(2) and (b)(2) rows in particular.
...
| 12 CFR 1002.9(b)(2)<br>`ecoa_reg_b_1002_9_b_2_specific_reasons` | The statement of reasons must be specific and indicate the principal reasons for the adverse action. | `record`: `present(artifact_logs_reason_explanation) and present(provenance_model_version) and present(scope_statements_local_vs_global)` | *Specific* and *principal*, which are the whole clause. A reason string of `"credit policy"` or `"n/a"` is present. `scope_statements_local_vs_global` is the pack author's proxy for specificity — a local explanation being about *this* decision rather than the model in general — and the regulation names nothing of the kind. The official commentary's requirement that the reasons relate to and accurately describe the factors actually scored, and its position on generic reason lists, is not formalised, and neither is the clause's own second sentence, which is not quoted in the pack: internal standards, policies and a failed qualifying score are *insufficient* reasons, and no engine here can tell an insufficient reason from a sufficient one. This property and the (a)(2) row above are now separable — a creditor capable of stating reasons that disclosed the right to request them instead satisfies (a)(2) and violates this one — but the separation exposes a limit of its own, and it is a false violation of the same family as the one (a)(2) carried: (b)(2) is triggered only where a statement of reasons is *required by paragraph (a)(2)(i)*, and that trigger is not modelled, so a creditor lawfully on branch (ii) who has not been asked for reasons is reported violated here. It is left standing rather than repaired because both available repairs are worse. Guarding the property on `present(artifact_logs_reason_explanation)` would make it *vacuously satisfied* for every system that gives no reasons at all — a system with nothing to be specific about would be reported as having been specific, which is the overclaim this tool exists to refuse. Modelling the trigger properly needs a signal for the applicant's request under (a)(2)(ii), which is a fact about correspondence after the notification and nothing in a decision record witnesses it. The honest repair is a signal for the request, not a rewrite of this formula. |
