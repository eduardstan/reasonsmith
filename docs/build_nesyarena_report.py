...
What a reader must not break:
  - `SOURCE_COMMIT` names the reasonsmith commit whose code produced the committed report, and
    is a literal rather than a `git rev-parse` for one reason: the report is committed *into*
    the tree it describes, so a live hash would name a commit that does not yet exist when the
    report is written and a different one on every later checkout, and `BUILD_COMMAND` would
    stop reproducing the file byte-for-byte. `test_nesyarena_report_matches_the_builder` runs
    this module and compares, so changing anything that moves the output fails the build until
    the report is regenerated and `SOURCE_COMMIT` is moved with it.
...
SOURCE_COMMIT = "ce30983fcc8e07c9529703a061918729681c5b3e"
