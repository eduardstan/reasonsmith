...
What a reader must not break:
  - `SOURCE_COMMIT` names the reasonsmith commit whose code produced the committed report, and
    is a literal naming the preceding builder-bearing commit rather than a `git rev-parse` for
    one reason: the report is committed *after* the builder it describes, so a live hash would
    name a commit that does not yet exist when the report is written and a different one on every
    later checkout, and `BUILD_COMMAND` would stop reproducing the file byte-for-byte.
    `test_nesyarena_report_matches_the_builder` runs this module and compares, while
    `test_builder_names_the_commit_that_produced_the_report` verifies that the named commit
    exists and contains this builder.
...
#: The reasonsmith commit whose code produced the committed report. See the module docstring
#: for why this is a literal.
SOURCE_COMMIT = "e99bbeb3ede2a4762371fb07fe6c1ad31a4feb3a"
...
def render() -> str:
    ...
    lines = [
        ...
        f"**reasonsmith commit:** `{SOURCE_COMMIT}`  ",
