...
carries, the graded readings, and one soundness statement per engine. It ends with the
repository's **bibliography**, which is a registry the build enforces rather than a list.
...
**What this document owns, and what the others keep.** It owns the **bibliography**: every citation
anywhere in this repository resolves to an entry below, and the mechanism that enforces that is
stated at the end. It does not own the operational documents. [`semantics.md`](semantics.md) states
