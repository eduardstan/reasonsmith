...
What this module is for:
  `docs/theory/07-explanation.md` is the definition and this is the measurement of it. The reason-
  ...
  *deletion lattice*, since it calls `without(fact)` and nothing else. That is a property of this
  module and not a limit of the protocol any more; `docs/theory/07-explanation.md` §7.1
  records the wider perturbation a family may offer and the fact that no definition here reads it.
  ...
  which is the same as lying in some AXp. `docs/theory/07-explanation.md` §7.3 carries the
  lemmas and states them in the repository's one notation, with the bibliography.
...
#: enumeration. A whole-space probe settles a truncating engine in one
#: (`docs/theory/07-explanation.md` §7.3, Corollary 7.12), and the loop below costs
#: on the order of `|space|` per contrastive set found,
...
def contrastive_sets(
    ...
    *,
    ...
    if space:
        try:
            # `docs/theory/07-explanation.md` §7.3, Corollary 7.12: if deleting everything
            # does not move the
