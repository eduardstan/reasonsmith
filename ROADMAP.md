...
is not: **the machinery exists and no shipped duty uses it**
...
- **Static registry — met:** [`docs/registry.html`](docs/registry.html) is the installed pack and
  engine inventory, built by [`docs/build_registry.py`](docs/build_registry.py) from entry-point
  discovery and byte-pinned by `tests/test_docs_registry.py`; the page states that discovery is
  the source and it is neither an endorsement nor an audit, and names entry points colliding with
  a built-in name rather than skipping them silently.
